...
- **Port**: 7865 (configurable via FOUNDRY_UI_PORT)
...
## Dependency: How Foundry uses MagicQuant

MagicQuant is available to Foundry through two mechanisms:
1. **Installed package**: `magicquant==0.1.0` is installed in the venv
2. **Symlink**: `MagicQuant/magicquant → /server/programming/MagicQuant/magicquant`

The UI's `do_magicquant()` adds `MagicQuant` to `sys.path` before importing. The CLI `pipeline.py` relies on the installed package or PYTHONPATH.

## unsloth-env Symlink Situation

**Status**: RESOLVED

The venv symlink skeleton has been replaced with a self-contained venv. See DEPLOY_SUMMARY.md for details.

### Key packages used by Foundry
...
- No import or runtime reference in either Foundry or `MagicQuant/`

Foundry migrated away from Unsloth to custom fast loaders (commits `cc18026`, `2c017f8`). This cache is a remnant.
...
### Foundry (11 files modified, 3 untracked)
...
### Foundry is 3 commits ahead of origin:
```
...
### Foundry: Variable name bug in hf_upload.py dry_run
...
### Foundry: Hardcoded values in fast_train_zeroclaw.py
...
### Foundry: DEVICE hardcoded to cuda:0
