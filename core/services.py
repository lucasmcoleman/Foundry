    ...
    def build_script(
        ...
        *,
        ...
        script += (
            ...
            f"    # Normalize: keep only 'messages' column with consistent schema\n"
            f"    if 'messages' not in _ds.column_names:\n"
            f"        raise ValueError(f'Dataset {{_src}} has no messages column: {{_ds.column_names}}')\n"
            f"    # Decode JSON-string messages if needed (some HF datasets store as List(Json))\n"
            f"    import json as _json\n"
            f"    def _norm(ex):\n"
            f"        msgs = ex['messages']\n"
            f"        if msgs and isinstance(msgs[0], str):\n"
            f"            ex['messages'] = [_json.loads(m) if isinstance(m, str) else m for m in msgs]\n"
            f"        return ex\n"
            f"    _ds = _ds.map(_norm)\n"
            f"    # Drop extra columns so schemas align for concatenation\n"
            f"    _keep = {{'messages'}}\n"
            f"    _drop = [c for c in _ds.column_names if c not in _keep]\n"
            f"    if _drop:\n"
            f"        _ds = _ds.remove_columns(_drop)\n"
            ...
            f"    # Cast all to same features before concat\n"
            f"    from datasets import Features, Sequence, Value\n"
            f"    _target_features = Features({{'messages': [{{'role': Value('string'), 'content': Value('string')}}]}})\n"
            f"    _loaded = [d.cast(_target_features) for d in _loaded]\n"
