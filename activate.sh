#!/bin/bash
# Activate the Unsloth training environment
#
...
echo "Unsloth environment activated (Python $(python --version 2>&1 | cut -d' ' -f2))"
...
UI_PORT="${UNSLOTH_UI_PORT:-7865}"
if ! ss -tlnp 2>/dev/null | grep -q ":${UI_PORT} "; then
    "$SCRIPT_DIR/ui/run.sh" "$UI_PORT" > "$SCRIPT_DIR/ui/ui.log" 2>&1 &
    UNSLOTH_UI_PID=$!
    echo "Pipeline UI started on http://localhost:${UI_PORT} (pid $UNSLOTH_UI_PID)"
else
