#!/bin/bash
# Activate the Foundry training environment
#
...
echo "Foundry environment activated (Python $(python --version 2>&1 | cut -d' ' -f2))"
...
# Start the Foundry UI in the background
UI_PORT="${FOUNDRY_UI_PORT:-7865}"
if ! ss -tlnp 2>/dev/null | grep -q ":${UI_PORT} "; then
    if [ -x "$SCRIPT_DIR/ui/run.sh" ]; then
        ...
        FOUNDRY_UI_PID=$!
        sleep 1
        if kill -0 "$FOUNDRY_UI_PID" 2>/dev/null; then
            echo "Foundry UI started on http://localhost:${UI_PORT} (pid $FOUNDRY_UI_PID)"
        else
            echo "WARNING: Foundry UI failed to start. Check $SCRIPT_DIR/ui/ui.log"
        fi
    else
        echo "WARNING: ui/run.sh not found or not executable"
    fi
else
    echo "Foundry UI already running on http://localhost:${UI_PORT}"
fi
